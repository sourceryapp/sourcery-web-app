	...
	export default {
        ...
        computed: {
            imgSrc: function(){
                ...
                return `${this.base}?center=${Utils.addressToEncodedString(this.repository)}&zoom=${this.zoom}&size=${this.width}x${this.height}&markers=color:blue|label:${this.repository.name}|${this.repository.geo._lat},${this.repository.geo._long}&key=${process.env.GOOGLE_MAPS_API_KEY}`;
            }
        }
	}
</script>
