    ...
    <div id="content-wrap" class="px-4">
      ...
      <v-layout style="margin-top: 84px">
        <v-flex xs12 sm6 xl4 offset-sm3 offset-xl4>
          ...
          <v-card v-for="(member, index) in members.sort((a, b) => (a.name.split(' ').pop() > b.name.split(' ').pop()) ? 1 : -1)" :key="member.name" outlined class="mb-4 pa-4">
            <v-row>
              <v-col :order-sm="index % 2" class="flex-grow-0" or>
                <v-img
          ...
          <div class="d-flex flex-wrap mx-sm-n8 justify-center justify-sm-space-between">
            <v-card
              v-for="member in alumni.sort((a, b) => (a.name.split(' ').pop() > b.name.split(' ').pop()) ? 1 : -1)"
              :key="member.name"
              color="transparent"
              flat
              class="mb-4 flex-shrink-1 mx-sm-4 pa-3"
              width="176"
            >
              <v-img
                :src="'/img/team/' + member.name.split(' ').pop() + '.jpg'"
