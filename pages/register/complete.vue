  ...
  <v-layout align-center>
    <v-flex xs12 sm6 offset-sm3 lg4 offset-lg4>
      ...
      <template v-if="isResearcher">
        ...
        <p>
          To do this, you'll need to add a credit card to your account.
          <v-dialog
            v-model="dialog"
            lazy
            max-width="500"
      ...
      <template v-if="isSourcerer">
        ...
        <p>
          To do this, you'll need to add a debit card (or bank info) to your account.
          <v-dialog
            v-model="dialog"
            lazy
            max-width="500"
      ...
      <div class="hidden-xs-only">
        <connect-button v-if="isSourcerer" class="v-btn--large v-btn--block v-btn--depressed" @click="markComplete" />
        <v-btn
          v-if="isResearcher"
          block
          large
          depressed
          color="primary"
          :to="{name: 'account-credit-cards'}"
          @click="markComplete"
        >
          Add Card
        </v-btn>
        <v-btn block large color="primary" outline @click.prevent="skip">
          Skip for Now
        </v-btn>
      </div>
...
export default {
     ...
     */
    middleware ({ store, $axios, redirect, route, error }) {
