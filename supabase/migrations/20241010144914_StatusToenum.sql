...
-- Create the payment_details table with a reference to the user table
...
-- Enable Row Level Security on the payment_details table
ALTER TABLE public.payment_details ENABLE ROW LEVEL SECURITY;

-- Grant select access to the user referenced in payment_details
DROP POLICY IF EXISTS "Select Own Payment Details" ON public.payment_details;
DROP POLICY IF EXISTS "Update Own Payment Details" ON public.payment_details;
DROP POLICY IF EXISTS "Select Payment Details for Admin" ON public.payment_details;
DROP POLICY IF EXISTS "Insert Own Payment Details" ON public.payment_details;
CREATE POLICY "Select Own Payment Details"
...
-- Grant update access to the user referenced in payment_details
CREATE POLICY "Update Own Payment Details"
ON public.payment_details
AS permissive
FOR update
TO authenticated
...
-- Grant select access to users with admin role
CREATE POLICY "Select Payment Details for Admin"
ON public.payment_details
AS permissive
FOR select
TO authenticated
USING ((SELECT admin FROM public.user WHERE id = auth.uid()));

-- Grant insert access to the user referenced in payment_details
CREATE POLICY "Insert Own Payment Details"
ON public.payment_details
AS permissive
FOR insert
TO authenticated
...
-- Step 2: Add the new status column to the requests table
ALTER TABLE requests ADD COLUMN pricing pricing_enum DEFAULT 'PRICING_FREE'::pricing_enum;


DROP FUNCTION IF EXISTS claim_request(bigint,pricing_enum);
DROP FUNCTION IF EXISTS claim_request(bigint,uuid);
CREATE OR REPLACE FUNCTION public.claim_request(input_request_id bigint, input_pricing pricing_enum)
RETURNS void LANGUAGE plpgsql security definer set search_path = 'public' AS $$
BEGIN
    UPDATE public.requests
    SET servicer_id = auth.uid(),
        servicer_claimed_at = NOW(),
        status = CASE input_pricing
            WHEN 'PRICING_FREE' THEN 'STATUS_PAID'::status_enum
            WHEN 'PRICING_SMALL' THEN 'STATUS_UNPAID'::status_enum
            WHEN 'PRICING_MEDIUM' THEN 'STATUS_UNPAID'::status_enum
            WHEN 'PRICING_LARGE' THEN 'STATUS_UNPAID'::status_enum
        END,
        pricing = input_pricing::pricing_enum
    WHERE id = input_request_id
      AND servicer_id IS NULL
      AND status = 'STATUS_CREATED'
      AND public_can_claim = TRUE;
...
BEGIN
    ...
    SET servicer_id = NULL,
        servicer_claimed_at = NULL,
        status = 'STATUS_CREATED'::status_enum,
        pricing = 'PRICING_FREE'::pricing_enum
    WHERE id = input_request_id
      AND servicer_id = auth.uid()
      AND status IN ('STATUS_PAID', 'STATUS_UNPAID');
END;
$$;